...
class ExtractCentroid:
    ...
    def extract_centroid(self):
        ...
        mem_layer = QgsVectorLayer(uri, 'centroid_points', 'memory')
        ...
        # print oldattributeList
        newattributeList = []
        # print newattributeList
        for attrib in oldattributeList:
            if mem_layer.fieldNameIndex(attrib.name()) == -2:
                #        print "attribute find"
                #        print  mem_layer.fieldNameIndex(attrib.name()) == True
                newattributeList.append(QgsField(attrib.code(), attrib.type()))
        prov.addAttributes(newattributeList)
        # print prov.addAttributes(newattributeList)
        ...
        #    print pt.y()
            ...
            y = pt.y()
            print x, y
            code_name = f.attribute("code")
            print code_name
        #    centroid_feature = QgsFeature(f.fields())
        #    print centroid_feature
        #    centroid_feature['code'] = code
        #    print code

            feat.setAttributes([code_name, x, y])
